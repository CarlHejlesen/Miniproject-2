...
void winner(int **arena, int size_of_arena, int lines, int turn) {


   // horizontal_check(arena, size_of_arena, lines, turn); // [works]

//vertical_check(arena, size_of_arena, lines, turn); // [works]

//high_left_check(arena, size_of_arena, lines, turn);

low_left_check(arena, size_of_arena, lines, turn);

}
void horizontal_check(int **arena, int size_of_arena, int lines, int turn){
    ...
    if(winner>0){printf(" player%d won the game\n",winner);}

}
void vertical_check(int **arena, int size_of_arena, int lines, int turn){
    int points;
    int winner = 0;
    //here we run it as many times as we need
    for (int i = 0; i <= size_of_arena-1; i++) {
...
void high_left_check(int **arena, int size_of_arena, int lines, int turn){
    int points;
    int winner = 0;
    //here we run it as many times as we need
    for (int i = 0; i <= size_of_arena-1; i++) {
        for (int o = 0; o <= size_of_arena-1; o++) {
            ...
            if (arena[o][i] == turn) {//printf("test %d %d ", o, i);
                //here we secure that we wont check out of bounds by limiting the checker to inside the arena
                if (o + lines-1 <= size_of_arena-1&& i+lines-1<=size_of_arena-1) { //printf("\ndo i get here? ");
                    //this is our counter this will count up when the check equals turn
                    points = 0;
                    //here we run the checker as many times that there is lines for win
                    for (int p = 0; p <= lines-1; p++) {
...
void low_left_check(int **arena, int size_of_arena, int lines, int turn){
    int points;
    int winner = 0;
    //here we run it as many times as we need
    for (int i = 0; i <= size_of_arena-1; i++) {
